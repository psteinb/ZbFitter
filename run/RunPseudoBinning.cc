...
void RunnerConfig::parse(){
  ...
  int opt = 0;
  while( (opt = getopt(m_argc, m_argv, "d:o:c:m:t:E:M:D:T:R:i:h" ))!=-1 ){
...
void RunnerConfig::printHelp(){
  ...
  std::cout << "\t -R <maxRebin> upper limit of the rebinning loop" << std::endl;
...
void RunnerConfig::printConf(){
  ...
  std::cout << "[-R] maxRebin = "<< p_maxRebin << std::endl;
